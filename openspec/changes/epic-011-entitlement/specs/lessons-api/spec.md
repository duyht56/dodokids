...
`GET /lessons/today?childId=` SHALL lookup `child.progress.currentWeek` và `currentDay`, query lesson từ DB, resolve asset URLs trong payload, và trả về lesson đầy đủ. Nếu không có content trong DB SHALL trả về stub response với `isStub: true`. Endpoint SHALL được bảo vệ bởi entitlement guard: nếu child chưa unlock `currentWeek` theo plan/status, response SHALL là `403` với `{ requiresUpgrade: true }` và handler không chạy.
...
#### Scenario: Child chưa unlock tuần hiện tại
- **WHEN** `GET /lessons/today?childId=child_x`, child ở `status: 'trial'` và `currentWeek` ≥ 3
- **THEN** response `403 Forbidden` với `{ "requiresUpgrade": true }` và không trả lesson

### Requirement: GET /lessons/:lessonId trả về lesson cụ thể
`GET /lessons/:lessonId` SHALL trả về lesson đầy đủ theo `lessonId` (format `w{week}-d{day}-{subject}`), `404` nếu không tìm thấy hoặc status không phải `imported`. Endpoint SHALL được bảo vệ bởi entitlement guard dựa trên `week` parse từ `lessonId`: nếu child chưa unlock tuần đó, response SHALL là `403` với `{ requiresUpgrade: true }`.
...
#### Scenario: Child chưa unlock tuần của lesson
- **WHEN** `GET /lessons/w03-d1-toan?childId=child_x` và child chưa unlock tuần 3
- **THEN** response `403 Forbidden` với `{ "requiresUpgrade": true }`
