...
- `takeBalanced` in `mobile/src/explore/variety.ts` served buckets round-robin
  ...
  for one exercise at a time, so they always got the FIRST bucket: "Bên nào
  nhiều hơn?" practically never asked "ít hơn"/"bằng nhau", "Máy cộng trừ" almost
  never subtracted past L1, "Khám phá số" never produced "sắp xếp thẻ".
- Distractors leaked answers: the nearest-values rule put the target in the
  ...
  number; Chạm và đếm printed `đã chạm/tổng` on its badge before the child
  counted.
...
- Shared distractor builder accepts `exclude[]` and is no longer median-biased;
  number (missing/before-after), arithmetic, count and compare use it.
...
- Catalog: pedagogical order (count → compare → numerals → pattern → memory →
  bond → arithmetic → route), four child-facing groups, larger thumbnails, no
  parent-facing copy, badge only when a game cannot be played.
- Per game (Đợt 1 scope only): tap_count stops leaking the total and counts
  aloud; arithmetic fixes the subtraction question, bans zero operands, offers
  typical-error distractors and locks wrong cards; pattern restores per-family
  spoken questions, single-line sequence, vocabulary-only distractors; number
  excludes visible/reference values and reads tapped numbers; compare reads
  counts aloud and gets its own asset pool; number_bond rotates prefilled parts,
  shuffles slots, reduces text and allows removing a leaf; route planner fits
  an iPhone SE, uses a D-pad and a distinct clear icon, shows the real Đô Đô;
  tracing (still hidden) decimates raw points by distance; memory follows BRD
  pair counts and runs three boards.
- Tests: `verify-explore-{number,compare,arithmetic}-contracts.cjs` added;
...
- `explore-number-game`, `explore-count-game`, `explore-quantity-compare-game`,
  `explore-arithmetic-game`, `explore-pattern-game`, `explore-number-bond-game`,
  `explore-memory-match-game`, `explore-tracing-workshop`,
  `explore-route-planner-game`: the per-game requirement changes listed above.

## Impact

- `mobile/src/explore/{variety,registry,audio,promptAudio}.ts`,
  `mobile/src/explore/games/{numberUtils,rangeProgress,arithmeticGame,memoryGame,…}.ts`,
...
- `docs/KIDO_EXPLORE_BRD.md` §7.5 (leaf removal by tap) and `docs/AI_CONTEXT.md`
