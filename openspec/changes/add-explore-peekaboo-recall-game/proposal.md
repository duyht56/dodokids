...
- A small set of distinct objects is shown in slots for a moment; Đô Đô covers
  them (peekaboo — NOT a countdown, there is no timer numeral); ONE object is
  taken away; the cover lifts to reveal the REMAINING objects plus one empty slot;
  and the child taps, from an options row, WHICH object is now missing. The empty
  slot's position is a spatial cue and the child recalls the object that was there.
...
- A renderer that plays the peekaboo (brief show → cover → reveal-with-gap →
  ...
  leaving a live distractor) and, at level 2, flashes the full set once as a
  memory aid; reduced-motion-friendly, native-driver motion, emoji-free UI.
- A progressive one-board-per-level L1→L5 run (like `missing_cell` and
  `odd_one_out`): no XP, streak, countdown or lose state; levels scale the set
...
- Mobile Explore only: a new `games/peekabooRecallGame.ts`, a new
  ...
  builder. Reuses the shared `MEMORY_ASSETS` object pool with no version change.
