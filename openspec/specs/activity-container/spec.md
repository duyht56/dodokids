# activity-container Specification
...
The container SHALL receive a single `Activity` object and render the matching component for its `actionType`: `single_select` → SingleSelectActivity, `multi_select` → MultiSelectActivity, `sort_sequence` → SortSequenceActivity, `match_pair` → MatchPairActivity, `count_tap` → CountTapActivity, `compare_tap` → CompareTapActivity, `audio_select` → AudioSelectActivity, `watch_video` → WatchVideoActivity. For `audio_select`, the container SHALL pass the activity id as the seed for stable layout/animation, as with the other seeded types.
...
#### Scenario: audio_select routes to AudioSelectActivity
- **WHEN** the container receives an activity with `actionType: 'audio_select'`
- **THEN** the AudioSelectActivity component is rendered with the normalized payload (each option's audioRef resolved to an audioUrl)
