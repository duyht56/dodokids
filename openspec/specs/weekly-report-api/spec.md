...
Defines the backend endpoint serving the parent weekly report — a gated, server-side report derived from the child's progress: per-subject completion, templated strengths/growth copy with a safety filter, a 4-week comparison series, and next-week suggestions. No external LLM is called; the response contract is shaped so a real LLM generator can drop in later. Child is identified by the `:childId` path parameter.
