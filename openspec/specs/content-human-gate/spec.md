...
The pipeline-admin WYSIWYG approval gate: reviewers preview each candidate activity rendered exactly as the mobile Lesson Player shows it (web replica, real assets/audio), confirm a production-ready checklist, and publish per week. It is the single human quality gate before content reaches kido-server — no AI review and no duplicate detection (Seed Review owns those upstream).
