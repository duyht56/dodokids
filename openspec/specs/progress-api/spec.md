...
`PATCH /progress/:childId/complete-lesson` SHALL nhận `{ lessonId, stars, activityResults }`, cập nhật `completedLessons`, tính `streakCount`, advance `currentDay`/`currentWeek`, xác định `stickerEarned` (nếu D5), invalidate Redis cache, trả về updated progress. Ngoài ra, endpoint SHALL lưu sao vào `progress.lessonStars` theo best-score semantics và trả `stars` trong response. XP/streak/advance chỉ áp dụng ở lần hoàn thành đầu (idempotent), nhưng sao MAY được nâng khi chơi lại.

#### Scenario: First completion stores stars and XP
- **WHEN** một lesson hoàn thành lần đầu với 3 sao
- **THEN** `lessonStars[lessonId]` thành 3, XP được cộng một lần, response chứa `stars: 3`

#### Scenario: Replay raises stars only
- **WHEN** một lesson đã hoàn thành được chơi lại với điểm sao cao hơn
- **THEN** `lessonStars[lessonId]` được nâng lên, response chứa `stars` mới, và `xpEarned` là 0

### Requirement: GET /progress/:childId/achievements
`GET /progress/:childId/achievements` SHALL trả về trạng thái gamification của child suy ra từ `progress`: `{ streakCount, weekDots[], badges[], stickers[] }`. Badges được derive (6 badge, thứ tự cố định), không lưu field mới. Child không tồn tại → `404`.

#### Scenario: Existing child
- **WHEN** `:childId` khớp một child tồn tại
- **THEN** response `200 OK` với object achievements

#### Scenario: Streak unlocks diligent badge
- **WHEN** `streakCount >= 7`
- **THEN** badge `diligent` có `earned: true`
