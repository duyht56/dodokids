...
Defines the `OfflineTaskCard` component — a reusable, controlled card for the week's at-home activity (Screen 14) with four visual states and action callbacks.
...
The component SHALL expose `onDone: () => void` and `onSkip: () => void` callback props. Both callbacks SHALL only be callable when `status === 'pending'`; in other states action buttons are not rendered.
