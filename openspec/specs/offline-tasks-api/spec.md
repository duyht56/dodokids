...
Defines the backend endpoints for the per-day weekly offline-task list (5 tasks/week, one per learning day) and per-task completion toggling, serving the parent Offline Tasks screen. Child is identified by the `:childId` path parameter.
