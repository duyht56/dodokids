...
The kido-server activity-management surface: a read-only view of content published into the runtime, with rollback/unpublish/re-publish controls (content stays read-only) and transfer monitoring that surfaces guard rejections.
